...
struct DataPoint
{
	cv::Mat image_depth, image_infrared;
	cv::Rect face_rect;
...
Transform Procrustes(const std::vector<cv::Point2d> &x, const std::vector<cv::Point2d> &y);

std::vector<cv::Point2d> MeanShape(std::vector<std::vector<cv::Point2d>> shapes, const TrainingParameters &tp);

std::vector<cv::Point2d> ShapeDifference(const std::vector<cv::Point2d> &s1, const std::vector<cv::Point2d> &s2);

std::vector<cv::Point2d> ShapeAdjustment(const std::vector<cv::Point2d> &shape, const std::vector<cv::Point2d> &offset);

double Covariance(double *x, double * y, const int size);

std::vector<cv::Point2d> MapShape(cv::Rect original_face_rect, const std::vector<cv::Point2d> original_landmarks,
...
std::pair<double, double> ComputePose(cv::Mat depth, const std::vector<cv::Point> & pixels_pos);
