...
TrainingParameters ReadParameters(const string &filename)
{
	...
	if (fin)
	{
		...
		result.Rho = stod(items.at("Rho"));
		if (result.Rho < 0 || result.Rho > 1)
			throw invalid_argument("Rho must be in [0, 1].");
		result.Alpha = stod(items.at("Alpha"));
		if (result.Alpha < 0 || result.Alpha > 1)
			throw invalid_argument("Alpha must be in [0, 1].");
		result.Kappa = stod(items.at("Kappa"));
		if (result.Kappa < 0.01 || result.Kappa > 1)
			throw invalid_argument("Kappa must be in [0.01, 1].");
